...
// One turn per conversation by default. alpha3's Claude harness loses its span
// exporter across a snapshot restore: a conversation's FIRST turn (restored
// from the golden snapshot) traces fully, every later turn (restored from its
// own snapshot) exports no Claude spans at all (measured: 6/6 vs 1/32). Scope
// turns need no memory, so a fresh conversation per turn keeps every trace
// whole (model, tokens, tools). SCOPE_TURNS_PER_SESSION > 1 trades that away.
const TURNS_PER_INSTANCE = Math.max(1, Number(process.env.SCOPE_TURNS_PER_SESSION) || 1);
...
export function enterprise({ apiBase, uiBase, token, atespace = 'kagent', log = () => {} }) {
  ...
  async function agents() {
    ...
    for (const t of r.agent_templates ?? []) {
      ...
      for (const h of v.status?.harnesses ?? []) {
        ...
        const tools = (v.spec?.tools ?? []).map(b => b.mcp?.server?.name).filter(Boolean);
        if (ready) list.push({ ns: t.ref.namespace, name: t.ref.name, harness: h.harness, tools,
  ...
  async function snapshot() {
    ...
    const agentRoutes = Object.fromEntries(fleet.map(a =>
      [a.name, { pool: a.pool, harness: a.harness, selector: a.selector, tools: a.tools }]));
